...
public class ProductDao {
	...
	public Object getTotalCount() throws SQLException {
		QueryRunner qr = new QueryRunner(C3P0Utils.getDataSource());
		String sql="select count(*) from product";
		Object totalCountObj = qr.query(sql, new ScalarHandler());
	...
	public List<Product> findProductByLimit(int pageIndex, int currentCount) throws SQLException {		
		...
		List<Product> pageData = qr.query(sql, new BeanListHandler<Product>(Product.class),pageIndex,currentCount);
		return pageData;
	}
	...
	public List<Product> findProductByWord(String word) throws SQLException {
		...
		List<Product> productList=qr.query(sql,new BeanListHandler<Product>(Product.class),word+"%",0,5);
		return  productList;
	}
